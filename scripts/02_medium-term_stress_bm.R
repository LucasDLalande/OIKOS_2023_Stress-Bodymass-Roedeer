...
dataCH <- data[data$pop=="C",]
cor.test(dataCH$cohorte, dataCH$qualite_cohorte, method="spearman")
?cor.test
dataTF <- data[data$pop=="3F",]
cor.test(dataTF$cohorte, dataTF$qualite_cohorte, method="spearman")
...
# First year
mmc <- lm(r_mass ~ scale(log(FGM1), scale=F)*scale(mass1, scale=F) + scale(log(FGM1), scale=F)*scale(quali_coh, scale=F) 
            + scale(log(FGM1), scale=F)*pop + scale(log(FGM1), scale=F)*sex, data = datagcn1)
summary(mmc)
options(na.action="na.fail")
s1fgm1 <- dredge(mmc, rank="AICc", evaluate=T)
s1fgm1
write.table(s1fgm1, "results/model selection tables/mediumFGM1grow.csv", row.names = F, sep=";", dec=".")


mod1 <- lmer(r_mass ~ (1|cohorte), data = datagcn1)
summary(mod1)
-0.3472-1.44*0.3023;-0.3472+1.44*0.3023
r.squaredGLMM(mod1)

# with high FGM value
mmc1a <- lm(r_mass ~ scale(log(FGM1), scale=F)*scale(mass1, scale=F) + scale(log(FGM1), scale=F)*scale(quali_coh, scale=F) 
            + scale(log(FGM1), scale=F)*pop + scale(log(FGM1), scale=F)*sex, data = datagcn1a)
summary(mmc1a)
options(na.action="na.fail")
s1fgm1high <- dredge(mmc1a, rank="AICc", evaluate=T)
s1fgm1high
write.table(s1fgm1high, "results/model selection tables/mediumFGM1growhigh.csv", row.names = F, sep=";", dec=".")

mod1a <- lmer(r_mass ~ (1|cohorte), data = datagcn1a)
summary(mod1a)
-0.3195-1.44*0.3140;-0.3195+1.44*0.3140

r.squaredGLMM(mod1a)

# mean
mmc <- lmer(r_mass ~ scale(log(mean_fgm), scale=F)*scale(mass1, scale=F) + scale(log(mean_fgm), scale=F)*scale(quali_coh, scale=F) + 
            scale(log(mean_fgm), scale=F)*pop + scale(log(mean_fgm), scale=F)*sex + (1|cohorte), data = datagcmean)
summary(mmc)
options(na.action="na.fail")
s1fgmmean <- dredge(mmc, rank="AICc", evaluate=T)
s1fgmmean

dd=subset(s1fgmmean, delta<2)
...
avgm <- model.avg(dd,subset=c("144", "72", "40", "8", "6", "140"), fit=T)
avgm#[["coefficients"]]
summary(avgm)
confint(avgm, level=0.85)

write.table(s1fgmmean, "results/model selection tables/mediumFGMmeangrow.csv", row.names = F, sep=";", dec=".")


mod1 <- lmer(r_mass ~ (1|cohorte), data=datagcmean)
summary(mod1)
-0.3731-1.44*0.3982;-0.3731+1.44*0.3982
...
# With high FGM value
mmca <- lm(r_mass ~ scale(log(mean_fgm), scale=F)*scale(mass1, scale=F) + scale(log(mean_fgm), scale=F)*scale(quali_coh, scale=F) + 
              scale(log(mean_fgm), scale=F)*pop + scale(log(mean_fgm), scale=F)*sex, data = datagcmeana)
summary(mmca)
options(na.action="na.fail")
s1fgmmeanhigh <- dredge(mmca, rank="AICc", evaluate=T)
s1fgmmeanhigh
write.table(s1fgmmeanhigh, "results/model selection tables/mediumFGMmeangrowhigh.csv", row.names = F, sep=";", dec=".")

dd=subset(s1, delta<2)
dd


mod2 <- lmer(r_mass ~ (1|cohorte), data=datagcmeana)
summary(mod2)
-0.3397-1.44*0.4133;-0.3397+1.44*0.4133
...
############################### REVIEW OIKOS ###################################
# First year
mmc <- lmer(r_mass ~ scale(log(FGM1), scale=F)*scale(quali_coh, scale=F)*pop*sex + scale(log(FGM1), scale=F)*scale(mass1, scale=F)
          + (1|cohorte), data = datagcn1, REML=F)
summary(mmc)
options(na.action="na.fail")
s1fgm1 <- dredge(mmc, rank="AICc", evaluate=T, trace=2)
s1fgm1
write.table(s1fgm1, "results/Review OIKOS/medium_term-growing1.csv", row.names = F, sep=";", dec=".")

# Mean FGM
mmc <- lmer(r_mass ~ scale(log(mean_fgm), scale=F)*scale(quali_coh, scale=F)*pop*sex + scale(log(mean_fgm), scale=F)*scale(mass1, scale=F)
            + (1|cohorte), data = datagcmean, REML=F)
summary(mmc)
options(na.action="na.fail")
s1fgmmean <- dredge(mmc, rank="AICc", evaluate=T, trace=2)
s1fgmmean
write.table(s1fgmmean, "results/Review OIKOS/medium_term-growingmean.csv", row.names = F, sep=";", dec=".")
...
unique(factor(datagcn1$id)) # 131 obs / 90 ind - 71 obs/ind
datagcn2 <- rbind(FCHgcn2, MCHgcn2, FTFgcn2, MTFgcn2)
unique(factor(datagcn2$id)) # 147 obs / 101 ind
datagcmean <- rbind(FCHgcmean, MCHgcmean, FTFgcmean, MTFgcmean)
unique(factor(datagcmean$id)) # 91 obs / 71 ind - 51 obs/ind

# First year
mmc1 <- lm(r_mass ~ scale(log(FGM1), scale=F)*scale(mass1, scale=F) + scale(log(FGM1), scale=F)*scale(quali_coh, scale=F) 
            + scale(log(FGM1), scale=F)*pop + scale(log(FGM1), scale=F)*sex, data = datagcn1)
summary(mmc1)
options(na.action="na.fail")
s1fgmt <- dredge(mmc1, rank="AICc", evaluate=T)
s1fgmt

dd=subset(s1fgmt, delta<2)
dd
...
write.table(s1fgmt, "results/model selection tables/mediumFGMtprime.csv", row.names = F, sep=";", dec=".")

mod1 <- lmer(r_mass ~ (1|cohorte), data = datagcn1)
summary(mod1)
-0.2402-1.44*0.1944;-0.2402+1.44*0.1944
r.squaredGLMM(mod1)


# mean
mmc <- lm(r_mass ~ scale(log(mean_fgm), scale=F)*scale(mass1, scale=F) + scale(log(mean_fgm), scale=F)*scale(quali_coh, scale=F) 
             + scale(log(mean_fgm), scale=F)*pop + scale(log(mean_fgm), scale=F)*sex, data = datagcmean)

summary(mmc)
options(na.action="na.fail")
s1fgmmeant <- dredge(mmc, rank="AICc", evaluate=T)
s1fgmmeant
write.table(s1fgmmeant, "results/model selection tables/mediumFGMmeantprime.csv", row.names = F, sep=";", dec=".")
...
mod1 <- lmer(r_mass ~ (1|cohorte), data=datagcmean)
summary(mod1)
-0.3093-1.44*0.32518;-0.3093+1.44*0.2518
...
# First year
mmc <- lmer(r_mass ~ scale(log(FGM1), scale=F)*scale(quali_coh, scale=F)*pop*sex + scale(log(FGM1), scale=F)*scale(mass1, scale=F)
           + (1|annee1), data = datagcn1, REML=F)
summary(mmc)
options(na.action="na.fail")
s1fgm1 <- dredge(mmc, rank="AICc", evaluate=T, trace=2)
s1fgm1
write.table(s1fgm1, "results/Review OIKOS/medium_term-prime1.csv", row.names = F, sep=";", dec=".")

# Mean FGM
mmc <- lmer(r_mass ~ scale(log(mean_fgm), scale=F)*scale(quali_coh, scale=F)*pop*sex + scale(log(mean_fgm), scale=F)*scale(mass1, scale=F)
            + (1|annee1), data = datagcmean, REML=F)
summary(mmc)
options(na.action="na.fail")
s1fgmmean <- dredge(mmc, rank="AICc", evaluate=T, trace=2)
s1fgmmean
write.table(s1fgmmean, "results/Review OIKOS/medium_term-primemean.csv", row.names = F, sep=";", dec=".")
